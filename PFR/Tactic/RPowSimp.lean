...
@[target] theorem atom_pf (a : ℝ) : a = a * 1 := by sorry

theorem atom_pf' {a a' : ℝ} (p : a = a') : a = a * 1 := by simp [*]
@[target] theorem atom_pow_pf (a : ℝ) : a = a ^ (1 : ℝ) * 1 := by sorry
...
* `a = a ^ 1 * 1 + 0`
-/
def evalAtom (e : Q(ℝ)) : AtomM (Result ExProd e) := by sorry


@[target] theorem mul_pf_left {a₂ b c : ℝ} (a₁ : ℝ) (_ : a₂ * b = c) : (a₁ * a₂ : ℝ) * b = a₁ * c := by
  sorry


@[target] theorem mul_pf_right {a b₂ c : ℝ} (b₁ : ℝ) (_ : a * b₂ = c) : a * (b₁ * b₂) = b₁ * c := by
  sorry


@[target] theorem mul_pp_pf_overlap {a₂ b₂ c x : ℝ} (ea eb : ℝ) (h : 0 < x) (_ : a₂ * b₂ = c) :
...
@[target] theorem pow_pos {a b e : ℝ} (ha : 0 < a) (hb : 0 < b) : 0 < a ^ e * b := by sorry


@[target] theorem pow_pf {a b b' e₁ e₂ : ℝ} (ha : 0 < a) (hb : 0 < b) (_ : b ^ e₂ = b') :
    (a ^ e₁ * b) ^ e₂ = a ^ (e₁ * e₂) * b' := by
  sorry


def evalPow {a : Q(ℝ)} (va : ExProd a) (e : Q(ℝ)) : Option (Q(0 < $a) × Result ExProd q($a ^ $e)) := by sorry


@[target] theorem pow_congr {a a' b c : ℝ} (_ : a = a') (_ : a' ^ b = c) : a ^ b = c := by sorry


@[target] theorem inv_congr {a a' b : ℝ} (_ : a = a') (_ : a' ^ (-1 : ℝ) = b) : (a⁻¹ : ℝ) = b := by
  sorry


@[target] theorem npow_congr {a a' c : ℝ} {b : ℕ} (_ : a = a') (_ : a' ^ (b : ℝ) = c) :
    Monoid.npow b a = c := by sorry


def rewrite (parent : Expr) (root := by sorry
...
/-- Elaborator for the `rpow_ring` tactic. -/
@[tactic rpowRingConv] def elabRPowRingConv : Tactic := by sorry
...
open Lean Lean.PrettyPrinter.Delaborator in
@[delab app.OfNat.ofNat] def delab_ofNat := by sorry
